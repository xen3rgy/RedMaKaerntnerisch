  ...
  if (elevenVoiceInput) elevenVoiceInput.value = elevenConfig.voice;
    ...
    function openSettings() {
      ...
      if (window.Admin && !window.Admin.isAdmin()) {
        showToast('API Setup ist nur im Admin-Modus verfügbar.', 'error');
        return;
      }
...
function buildExamplesText() {
  ...
  // 1) Pre-enforce lexicon on user input
  const pre = window.RK_Lexicon ? window.RK_Lexicon.apply(text) : text;

  // Same prompt build as Dialekt-Chat
  let sysPrompt =
    DIALECT_SYSTEM_PROMPT && DIALECT_SYSTEM_PROMPT.trim().length > 0
      ? DIALECT_SYSTEM_PROMPT.trim()
      : 'Du bist ein hochpräziser Übersetzer für mittelkärntnerischen Dialekt.';
  ...
  // Translation-mode Klartext (damit er nicht “antwortet”, sondern wirklich übersetzt)
  sysPrompt += '\n\nMODUS: ÜBERSETZEN. Gib NUR den übersetzten Text zurück. Keine Erklärungen.';
  ...
  const raw = await callLLM(msgs, { purpose: 'translate' });
